...
The three-layer RL figures are the exception: `w_{walking,dancing,spinkick}_L3_rl.svg`
...
integer cell coordinates under a `translate/scale` group. Together: **7.4 MB -> 683 KB**
raw, 135 KB over gzip (GitHub Pages gzips by default), 40,093 DOM nodes -> about 260.
Renders faster than the matplotlib original and differs from it only by antialiasing on
shared cell edges.

Still rasterised, and worth re-exporting as SVG if you have the sources:
`w_*_L3_rlil.webp` (Fig. 12, RL vs IL across all three layers), `method.webp` (Fig. 2)
and `jointwise.webp` (Fig. 4b), where the text is small.
